...
public class TwitchClient {
    ...
    public final HashMap<String, OAuth2Credential> credentialManager = new HashMap<>();
    ...
    public TwitchClient(GenericSender p) {
        started.set(false);
        reloading.set(true);
        twitchExecutor = Executors.newSingleThreadExecutor();
        refreshExecutor = Executors.newSingleThreadScheduledExecutor();

        config = ChatPointsTTV.getInstance().config.getTwitchConfig();

        twitchExecutor.submit(() -> {
            try {
                CPTTV_EventHandler.clearActions(Platforms.TWITCH); // Make sure actions will be parsed again

                /*File twitchConfigFile = new File(ChatPointsTTV.getInstance().getDataFolder(), "twitch.yml");
                if (!twitchConfigFile.exists()) {
                    ChatPointsTTV.getInstance().saveResource(twitchConfigFile.getName(), false);
                }
                //twitchConfig = YamlConfiguration.loadConfiguration(twitchConfigFile);

                accountsFile = new File(ChatPointsTTV.getInstance().getDataFolder(), "accounts");
                ...
                if (ChatPointsTTV.getAccountsManager().hasPlatform(Platforms.TWITCH)) {
                    for (String userid : ChatPointsTTV.getAccountsManager().getAccounts(Platforms.TWITCH)) {
                ...
                ChatPointsTTV.log.error(e.getMessage());
                e.printStackTrace();
            }
        ...
        if (ChatPointsTTV.getInstance().config.getGeneralConfig().getBoolean("SHOW_CHAT", true)) {
            eventManager.onEvent(ChannelMessageEvent.class, event -> {
                ...
                if (!chatBlacklist.contains(event.getUser().getName())) {
                    ...
                    ChatComponent chatMsg = new ChatComponent(mcColor + event.getMessageEvent().getUserDisplayName().get() + ": " + ChatColor.RESET + event.getMessage());
        ...
        try {
            ...
            if (e.getCause() != null && e.getCause().getMessage() != null) {
                if (e.getCause().getMessage().contains("errorStatus=400")) { // Bad request. (Only possible cause is exceeding maximum rewards)
                    ...
                    ChatPointsTTV.log.warn("Twitch account " + account.getUserName() + " has not enrolled to the monetisation program. Therefore they cannot create Channel Point Rewards.");
                    return false;
                }
            }
    ...
    private OAuth2Credential getCredential(String userId) {
        if (!credentialManager.containsKey(userId)) {
            java.util.HashMap<String, String> creds = ChatPointsTTV.getAccountsManager().getCredentials(Platforms.TWITCH, userId);
            if (creds == null || !creds.containsKey("access_token") || !creds.containsKey("refresh_token")) {
                throw new NullPointerException("Couldn't retrieve credentials for user: " + userId);
            }
            return new OAuth2Credential(identityProvider.getProviderName(), creds.get("access_token"), creds.get("refresh_token"), userId, null, null, null);
        } else {
    ...
    private void removeAccount(String username) {
        ...
        tokenRefreshTasks.get(channel.getChannelId()).cancel(true);
    ...
    public void stop(GenericSender p) {
        stopThread = new Thread(() -> {
            ...
            try {
                ...
                ChatPointsTTV.log.warn("There was an error while disabling the Twitch Module.");
                e.printStackTrace();
                return;
            }
